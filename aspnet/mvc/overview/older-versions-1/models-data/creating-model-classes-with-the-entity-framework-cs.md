...
Speichern Sie Ihre Entity Data Model, nachdem Sie eine Änderung vorgenommen haben, indem Sie auf die Schaltfläche Speichern klicken (das Symbol der Diskette). Im Hintergrund generiert das Entity Designer einen Satz von C# Klassen. Sie können diese Klassen anzeigen, indem Sie die MoviesDBModel.Designer.cs-Datei im Projektmappen-Explorer Fenster öffnen.

Ändern Sie den Code in der Datei Designer.cs nicht, da die Änderungen bei der nächsten Verwendung des Entity Designer überschrieben werden. Wenn Sie die Funktionalität der in der Designer.cs-Datei definierten Entitäts Klassen erweitern möchten, können Sie *partielle Klassen* in separaten Dateien erstellen.
...
Um die Klasse "moviesdbentities" im Home-Controller verwenden zu können, müssen Sie den Namespace "movieentityapp. Models" (*mvcprojectname*) importieren. Modelle).

Das Feld \_DB wird innerhalb der Index ()-Aktion verwendet, um die Datensätze aus der Datenbanktabelle "Movies" abzurufen. Der Ausdruck \_DB. "Movieset" stellt alle Datensätze aus der Datenbanktabelle "Movies" dar. Die Methode "-Methode ()" wird verwendet, um den Satz von Filmen in eine generische Auflistung von Movie-Objekten (List&lt;Movie&gt;) zu konvertieren.
...
Die Ansicht in der Liste 2 enthält eine **foreach** -Schleife, die jeden Film Daten Satz durchläuft und die Werte der Titel-und Director-Eigenschaften des Movie-Datensatzes anzeigt. Beachten Sie, dass neben jedem Datensatz ein Link zum Bearbeiten und löschen angezeigt wird. Außerdem wird unten in der Ansicht ein Link zum Hinzufügen von Movie angezeigt (siehe Abbildung 7).

**Abbildung 7 – Index Ansicht**

![clip_image014](creating-model-classes-with-the-entity-framework-cs/_static/image7.jpg)

Die Index Sicht ist eine *typisierte Sicht*. Die Index Sicht enthält eine &lt;% @ Page%&gt;-Direktive mit einem *erbt* -Attribut, das die Model-Eigenschaft in eine stark typisierte generische Listen Auflistung von Movie-Objekten (List&lt;Movie) umwandelt.
...
**Codebeispiel 3 – controllers\homecontroller.cs (Methoden hinzufügen)**
...
**Codebeispiel 4 – controllers\homecontroller.cs (Methoden bearbeiten)**
