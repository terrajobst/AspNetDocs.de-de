...
---
# <a name="iteration-3--add-form-validation-c"></a>Iterations #3 – Formular Validierung hinzufügenC#()
...
[![des Dialog Felds "Neues Projekt"](iteration-3-add-form-validation-cs/_static/image1.jpg)](iteration-3-add-form-validation-cs/_static/image1.png)

**Abbildung 01**: ein Formular mit Validierung ([Klicken Sie, um das Bild in voller Größe anzuzeigen](iteration-3-add-form-validation-cs/_static/image2.png))
...
Beachten Sie den Aufruf der HTML. ValidationSummary ()-Hilfsmethode, die direkt über dem HTML-Formular angezeigt wird. Wenn Validierungs Fehlermeldungen vorliegen, zeigt diese Methode Validierungs Meldungen in einer Auflistungs Liste an.

Beachten Sie außerdem die Aufrufe von HTML. validationmessage (), die neben den einzelnen Formularfeldern angezeigt werden. Das Hilfsprogramm validationmessage () zeigt eine einzelne Validierungs Fehlermeldung an. Im Fall von "Listing 1" wird ein Sternchen angezeigt, wenn ein Validierungs Fehler vorliegt.

Zum Schluss rendert das Hilfsprogramm HTML. TextBox () automatisch eine Cascading Stylesheet-Klasse, wenn ein Validierungs Fehler vorliegt, der der vom Hilfsprogramm angezeigten Eigenschaft zugeordnet ist. Das Hilfsprogramm HTML. TextBox () rendert eine Klasse mit dem Namen **Input-Validation-Error**.

Wenn Sie eine neue ASP.NET MVC-Anwendung erstellen, wird automatisch ein Stylesheet mit dem Namen "Site. CSS" im Inhalts Ordner erstellt. Dieses Stylesheet enthält die folgenden Definitionen für CSS-Klassen, die mit der Darstellung von Validierungs Fehlermeldungen verknüpft sind:
...
**Codebeispiel 2: controllers\contactcontroller.cs (Create with Validation)**
