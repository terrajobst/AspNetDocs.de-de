...
Im Gegensatz zu ASP.NET oder Active Server Pages enthält ASP.NET MVC nicht alles, der direkt auf eine Seite entspricht. In einer ASP.NET MVC-Anwendung besteht keine Seite auf dem Datenträger, der den Pfad in der URL entspricht, die Sie in die Adressleiste des Browsers eingeben. Am ehesten mit einer Seite in einer ASP.NET MVC-Anwendung ist etwas bezeichnet ein *Ansicht*.
...
Verwenden Sie die Skript-Trennzeichen &lt;% "und"&gt; Anfang und Ende eines Skripts zu markieren. Dieses Skript ist in Visual Basic geschrieben. Es zeigt das aktuelle Datum und die Uhrzeit durch Aufrufen der Response.Write()-Methode, um Inhalt an den Browser zu rendern. Die Skript-Trennzeichen &lt;% "und"&gt; können verwendet werden, um eine oder mehrere Anweisungen ausführen.
