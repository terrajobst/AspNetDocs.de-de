...
**Tabelle 1:** Welche Dateien, die Sie bereitstellen, hängt von das Kompilierungsmodell verwendet.
...
Wenn Microsoft Visual Studio 2005 veröffentlicht, die sie Unterstützung für das Webanwendungsprojekt-Modell und mit dem Websiteprojekt-Modell ersetzt. Das Websiteprojekt-Modell unterscheidet sich selbst von das Webanwendungsprojekt-Modell gibt folgende Möglichkeiten:
...
- Drei verschiedener CSS-Dateien im Ordner "Stile".
...
Das Webanwendungsprojekt-Modell verwendet explizite Kompilierung - Quellcode des Projekts kompiliert wird, in eine einzelne Assembly jedes Mal, wenn Sie die Anwendung zu erstellen. Diese Kompilierung enthält die ASP.NET-Seiten Code-Behind-Dateien (~ /`Default.aspx.cs`, ~ /`About.aspx.cs`und so weiter), als auch die `BasePage.cs` Klasse. Die resultierende Assembly BookReviewsWAP.dll heißt und befindet sich in der Anwendung `Bin` Verzeichnis.
...
Sie müssen sich nicht um die ASP.NET-Seiten Quellcodedateien in der produktionsumgebung zu kopieren, Sie brauchen auch zum Kopieren der `BasePage.cs` Klassendatei.
...
- [Projektsystem für Visual Studio 2005-Website: Worum handelt sich es, und warum haben wir es getan?](https://weblogs.asp.net/scottgu/archive/2005/08/21/423201.aspx)
