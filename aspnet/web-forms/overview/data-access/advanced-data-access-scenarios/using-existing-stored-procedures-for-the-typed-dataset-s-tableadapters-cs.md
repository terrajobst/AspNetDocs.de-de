...
Nachdem die `Products_SelectByCategoryID` gespeicherte Prozedur wurde in der Datenbank können wir Concigure unsere Data Access Layer, um diese gespeicherte Prozedur zu verwenden, wenn eine ihrer Methoden aufgerufen wird. Insbesondere fügen wir eine `GetProducstByCategoryID(categoryID)` Methode, um die `ProductsTableAdapter` in die `NorthwindWithSprocs` typisierte DataSet, die aufruft, die `Products_SelectByCategoryID` gespeicherte Prozedur, die wir gerade erstellt haben.
