...
---
<a name="1160--2016-12-01"></a>1.16.0/1.12.2016
==================

## <a name="additional"></a>Zusätzliche
  * Optimieren von cifES- und nieES-Algorithmen Schließt #1826

## <a name="build"></a>Build
  * Einschließen der minimierten Version in NPM-Paket
  * Ändern von Dev-Abhängigkeiten auf die aktuellen Versionen

## <a name="core"></a>Kernspeicher
  * Hinzufügen von Bindung für die Eingabe mit dem Schaltflächentyp. Schließt #1891.
  * Unterstützung für jquery3. Schließt #1866.
  * Ändern von jQuery-Alias 'expr[":"]' in 'expr.pseudos'

## <a name="localisation"></a>Lokalisierung
  * Hinzufügen der Urdu-Übersetzung. Schließt #1873.

## <a name="localization"></a>Lokalisierung
  * Korrigieren der falschen Dateierweiterung für az-Übersetzung. Schließt #1890.
  * Hinzufügen einer fehlenden Übersetzung in pt-BR (Schließt #1897)
  * Korrigieren des Schreibfehlers in arabischer-Sprachdatei.

## <a name="tests"></a>Tests
  * Aktualisieren von QUnit auf 2.0.

## <a name="umd"></a>UMD
  * Bessere Unterstützung für CommonJS.

<a name="1151--2016-07-22"></a>1.15.1/22.7.2016
==================

## <a name="additional"></a>Zusätzliche
  * Korrigieren mehrerer Mime-Typ-Validierungen
  * IBAN erfordert mindestens 5 Zeichen (Schließt #1797, Behebt #1674)
  * Korrigieren von NotEqualTo-jQuery-Verweis

## <a name="core"></a>Kernspeicher
  * Hinzufügen eines fehlerhaften Tests für #1805
  * Korrigieren der Gruppenvalidierung mit 3 und mehr Feldern
  * Korrigieren von in #1644 und #1657 eingeführten Regressionen (Schließt #1800)
  * Aktualisieren der Schrittvalidierung zur korrekten Verarbeitung von Gleitkommazahlen
  * Beheben eines Fehler beim Aufruf von $.fn.rules() für ein form-fremden Element
  * Aufrufen von `errorPlacement` im Validierungssteuerelement-Bereich
  * Korrigieren eines Problems mit contenteditable-Elementen in Formularen, bei denen eine einzelne Eingabeüberprüfung Ausnahmen verursachen würde

## <a name="demo"></a>Demo
  * Hinzufügen von Links zu Bootstrap- und Semantik-UI-Demos für die index.html
  * Verwenden von `.on()` anstelle von `.validateDelegate()`

## <a name="localization"></a>Lokalisierung
  * Hinzufügen der aserbaidschanischen Sprache

## <a name="tests"></a>Tests
  * Hinzufügen von Regressionskomponententests für PR #1760

<a name="1150--2016-02-24"></a>1.15.0/24.2.2016
==================

## <a name="all"></a>Alle
  * Korrigieren von Codeformatproblemen

## <a name="core"></a>Kernspeicher
  * `resetForm` sollte auch die `valid`-Klasse aus Elementen entfernen.
  * Aufheben der Markierung des Felds, wenn bei der Verwendung der Remoteregel bereits markiert.
  * Einmaliges Binden des `blur`-Ereignisses in der `equalTo`-Regel
  * Korrigieren eines Fehlers beim Aufrufen von .rules() in leerem jquery-Satz.
  * Korrigieren der Bearbeitung von Fehlermeldungen mit Eingabegruppen.
  * Beheben von TypeError in `showLabel` bei der Verwendung der `groups`-Einstellungen
  * Hinzufügen einer Möglichkeit zum Übergeben des Methodennamens an Remote
  * Die Validierung wird nicht ausgelöst, wenn das nächste Feld bereits ausgefüllt ist (Korrigiert #1508).
  * Erforderliche Regel hat Vorrang vor Zahlen- und Ziffernregeln
  * Fehler ausgeblendet, aber Eingabefehlerklasse nicht entfernt
  * Remotevalidierung verwendet falsche Fehlermeldungen
  * Korrigieren der Feldmarkierung mit Remotevalidierung
  * Korrigieren des `:filled`-Selektors für mehrere Select-Elemente.
  * Hinzufügen von Dokumentationsverweis zu jQuery.validator.methods
  * Verschieben der Nachrichtenverarbeitung von `formatAndAdd` zu `defaultMessage`
  * ErrorList sollte nur die Fehler enthalten, die enthalten sein sollten.
  * Extrahieren des Dateinamen ohne Angabe von „C:\fakepath\"
  * Unterstützung für HTML5-Schrittattribut. Behebt #1295
  * Unterstützung für Klasse „Ausstehend“ für ausstehende Anforderungen
  * Hizufügen des Normalizer (#1602)
  * Teilen der `creditcard`-Methode
  * Escape für errorID zur Verwendung in regulärem Ausdruck, nicht zum Aufbau von aria-describedby
  * Escape für einzelne Anführungszeichen in Namen zur Vermeidung der Ausgabe eines Sizzle-Fehlers
  * Serialisiertes Datenobjekt der Anfrage verwenden, anstatt den Wert des Feldes zu validieren, um einen API-Aufruf zu überspringen
  * Hinzufügen von Unterstützung für contentEditable-Tags

## <a name="additional"></a>Zusätzliche
  * BIC: Erlauben der Ziffern 1-9 an zweiter Stelle der Lokalisierung
  * Regulärer Ausdruck der Accept-Methode muss ordnungsgemäß mit Escapezeichen versehen sein
  * Überprüfung der Groß-/Kleinschreibung für BIC
  * Korrigieren von postalCodeCA zum Ausschließen ungültiger Kombinationen
  * Macht die postalCodeCA-Methode flexibler.

## <a name="localization"></a>Lokalisierung
  * Hinzufügen der mazedonischen Lokalisierung.
  * Hinzufügen der fehlenden Musternachricht in polnischer Sprache (Adamwojtkiewicz)
  * Korrigieren der persischen Übersetzung von Min-/Max-Nachricht.
  * Aktualisieren der messages_sk.js
  * Aktualisieren der malaiischen Übersetzung
  * Einschließen von Nachrichten aus zusätzlichen Methoden
  * Verbessern der pt_BR-Übersetzung und Beheben eines Schreibfehlers im cifES-Schlüssel

<a name="1140--2015-06-30"></a>1.14.0/30.6.2015
==================
...
## <a name="core"></a>Kernspeicher
  ...
  * Nicht gesamtes DOM nach Optionsfeld-/Kontrollkästchenelementen durchsuchen.
...
==================
  ...
  * Veranlassen, dass valid() einen booleschen Wert anstelle von 0 oder 1 zurückgibt. Behebt #109: valid() gibt keinen booleschen Wert zurück.
...
* form.elements nicht verwenden, weil dies in jQuery 1.6 nicht mehr unterstützt wird. Sowieso schwerwiegend fehlerhaft (IE6-8: form.elements === form).
...
* Korrigieren der Kreditkartenüberprüfung, damit nur Ziffern und Bindestriche angenommen werden („asdf“ ist keine gültige Kreditkartennummer).
...
* Korrigieren von optional-methods, damit nichts hervorgehoben wird, wenn das Feld leer ist, also kein success ausgelöst wird.
...
* Vollständiges Entfernen der Abhängigkeit von Element-IDs, obwohl diese weiterhin verwendet werden (sofern vorhanden), um Fehlerbezeichnungen mit Eingaben zu verknüpfen. Dies wurde erreicht, indem ein Array mit {name, message, element} anstelle eines Objekts mit id:message-Paaren für die interne Fehlerliste verwendet wird.
